...
export type ChunkedD1StoreOptions = {
  /**
...
export const DEFAULT_CHUNKED_D1_MAX_CHUNK_BYTES = 1_000_000;
...
const resolveMaxChunkBytes = (value: number | undefined): number => {
  const maxChunkBytes = value ?? DEFAULT_CHUNKED_D1_MAX_CHUNK_BYTES;
  if (
...
/**
 * opt-in D1 store for streams that can outgrow one Cloudflare SQL row.
 ...
 */
export class ChunkedD1Store implements StreamStore {
  ...
  constructor(db: D1Database, options?: ChunkedD1StoreOptions) {
    this.db = db;
    this.maxChunkBytes = resolveMaxChunkBytes(options?.maxChunkBytes);
  }

  async initialize(): Promise<void> {
    await new D1Store(this.db).initialize();
    await this.db.exec(ChunkedD1Store.schema);
    await this.db.exec(ChunkedD1Store.chunksByEndIndex);
  }
  ...
  private async prepareForkCreate(
    ...
    const sourceData = await this.readBytes(sourcePath, source);
  ...
  async get(path: string, options?: GetOptions): Promise<GetResult> {
    ...
    const messages = await this.readMessages(path, touched, startOffset);
  ...
  async waitForData(
    ...
    const messages = await this.readMessages(path, touched, offset);
  ...
  private async readBytes(
    ...
    const messages = await this.readMessages(path, stream, initialOffset());
