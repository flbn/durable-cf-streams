...
describe("ChunkedD1Store", () => {
  it("stores aggregate streams beyond the D1 row limit in bounded chunks", async () => {
  ...
  it("rejects snapshot D1Store payloads above the SQL value limit", async () => {
    const rejected = await command({
      op: "snapshotTooLarge",
      path: streamPath(),
      size: 2_000_001,
