...
export class StreamDO implements DurableObject {
  ...
  private async handleGet(
    ...
    const liveResult = parseLiveMode(url, request, offset);
    if (liveResult.mode === "error") {
      return liveResult.error;
    }

    if (liveResult.mode === "sse" && offset !== undefined) {
      return this.handleSSE(path, offset, cursorParam ?? undefined);
    }
    if (liveResult.mode === "long-poll" && offset !== undefined) {
